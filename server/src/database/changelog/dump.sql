...
CREATE TABLE IF NOT EXISTS `Techgo`.`Pedidos` (
  ...
  `trabalhadores_id` CHAR(32) NOT NULL,
  `usuarios_id` CHAR(32) NOT NULL,
  PRIMARY KEY (`id_pedido`, `trabalhadores_id`, `usuarios_id`),
    FOREIGN KEY (`trabalhadores_id`)
    REFERENCES `Techgo`.`trabalhadores` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
/*
insert into Techgo.Administrador values('uuid1', 'sergio', sha1('senha'),'2003-05-22');
...
insert into techgo.categoria values(null, 'eletricista', 'especialista em redes');
